...
function loadOrder(orderId: string, { database }: { database: Database }): Option<OrderFormDto> {
...
export function orderApi({ body }: { body: unknown }) {
  ...
  const existingFormEither = loadOrder(orderFormEither.right.OrderId, dependencies)

  //Pure code
  const validationEither = validateOrder(orderFormEither.right, existingFormEither);
